import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import api from '../../services/api';
import '../common/PageCommon.css';

const ContractList: React.FC = () => {
  const [contracts, setContracts] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const navigate = useNavigate();

  useEffect(() => {
    fetchContracts();
  }, []);

  useEffect(() => {
    const timer = setTimeout(() => {
      fetchContracts();
    }, 500);
    return () => clearTimeout(timer);
  }, [search]);

  const fetchContracts = async () => {
    try {
      const params: any = {};
      if (search) params.client_name = search;
      const response = await api.get('/contracts', { params });
      setContracts(response.data);
    } catch (error) {
      console.error('Failed to fetch contracts:', error);
    } finally {
      setLoading(false);
    }
  };

  const getStatusText = (status: string) => {
    const statusMap: { [key: string]: string } = {
      draft: '작성중',
      signed: '계약완료',
      in_progress: '진행중',
      completed: '완료',
      cancelled: '취소됨',
    };
    return statusMap[status] || status;
  };

  if (loading) return <div className="page-loading">로딩 중...</div>;

  return (
    <div className="page-container">
      <div className="page-header">
        <h1>계약 관리</h1>
        <button className="btn-primary" onClick={() => navigate('/contracts/new')}>
          + 계약 등록
        </button>
      </div>
      <div className="page-filters">
        <input
          type="text"
          placeholder="거래처명으로 검색..."
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="search-input"
        />
      </div>

      <div className="table-container">
        <table>
          <thead>
            <tr>
              <th>ID</th>
              <th>계약번호</th>
              <th>거래처</th>
              <th>상태</th>
              <th>총액</th>
              <th>작업</th>
            </tr>
          </thead>
          <tbody>
            {contracts.map((contract) => (
              <tr key={contract.id}>
                <td>{contract.id}</td>
                <td>{contract.contract_number}</td>
                <td>{contract.client?.name || contract.client_id}</td>
                <td>{getStatusText(contract.status)}</td>
                <td>{contract.total_amount?.toLocaleString()}원</td>
                <td>
                  <button className="btn-edit" onClick={() => navigate(`/contracts/${contract.id}`)}>
                    상세
                  </button>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  );
};

export default ContractList;

